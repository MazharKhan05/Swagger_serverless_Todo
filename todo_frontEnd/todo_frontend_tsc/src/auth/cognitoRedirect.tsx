...
const LoginRedirect = () => {
  ...
  console.log(params)  
  console.log(process.env.REACT_APP_COGNITO_CALLBACK_URL, "in here")
  ...
  useEffect(() => {
    const callTokenEndpoint = async (code:string)=>{
        ...
        console.log("response from token endpoint", awsAuthorizationCodeResponse);

        let expires = new Date()
        expires.setTime(expires.getTime() + (awsAuthorizationCodeResponse.data.expires_in * 1000))
        setCookie('id_token', awsAuthorizationCodeResponse.data.id_token, { path: '/',  expires})
        setCookie('access_token', awsAuthorizationCodeResponse.data.access_token, { path: '/',  expires})
        setCookie('refresh_token', awsAuthorizationCodeResponse.data.refresh_token, { path: '/',  expires})
